...
class JenaFuseki:
    def __init__(self, endpoint_url='http://localhost:3030/java_case/query'):
    ...
    def print_result_to_string(self, query_result):
        ...
        if query_head is None:
            ...
            for qr in query_result:
                for _, value in qr.iteritems(): #_,是unicode解码
                    print value, ' ',
                    tmp.append(value)
                print
        len=tmp.__len__()
        ...
        while(i<len):
            # list.append(tmp[i+1])
            print"{type: 'resolved', target: '", tmp[i+1], "', source: '", tmp[i], "', 'rela': '人员'},"
            i+=2
        # print list[1]
